...
const ProgLan = () => {
    ...
    const groupedFrameworks =
        stateFrameworks.length > 0
            ? [
                  {
                      language: langFromQuery || "Frameworks",
                      list: stateFrameworks,
                  },
              ]
            : List.map((lang) => ({
    ...
    return (
        <div className='container mx-auto md:px-24'>
            <div className='px-4 space-y-6 py-9 bg-base-100'>
                ...
                <p className='px-4 text-sm font-normal leading-relaxed tracking-wide text-justify md:px-20 text-base-content/80'>
                    {def}
                </p>
                <div className='flex justify-center mt-8'>
                    <Link
                        className='text-sm btn btn-outline no-animation md:text-base'
                        to='/'
                    >
                        Back
                    </Link>
            ...
            {groupedFrameworks.map(({ language, list }) => (
                ...
                >
                    {groupedFrameworks.length > 1 && (
                        <h2 className='mt-8 mb-4 text-6xl font-semibold text-left'>
                            {language}
                        </h2>
                    )}
                    <div className='flex flex-wrap items-start justify-center gap-4 pb-10 '>
                        {list.map((framework) => (
                            <Card
